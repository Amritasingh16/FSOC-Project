...
const createComment = async function (req, res) {
    try {
        ...
        let {comment} = data
        if(data.reply) return res.status(400).send({status:false,msg:"Cannot post replay on comment before posting comment"})
...
const postReply = async function(req,res){
    try {
        let data = req.body
        if(!data.commentId) return res.status(400).send({status:false,msg:"commentId is required"})
        if(!data.reply) return res.status(400).send({status:false,msg:"reply is required"})
        let {reply} = data
        let postReply = await commentModel.findOneAndUpdate({_id:data.commentId,isDeleted:false},{$push:{reply:reply}},{new:true})
...
module.exports = { createComment, updateComment, deleteComment, postReply }
